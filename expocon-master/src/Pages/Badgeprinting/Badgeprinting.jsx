  ...
  const allAttendees = async (searchQuery) => {
    try {
      const res = await axios.post(
        `https://expocon-master.vercel.app/api/v1//attendee/all-attendee`,
        { searchQuery }
      );
      if (res.data.success) {
  ...
  const updateAttendeePrintStatus = async (reg_number) => {
    try {
      ...
      const res = await axios.put(
        `https://expocon-master.vercel.app/api/v1//attendee/update`,
        {
          reg_number,
          badge_printed: true,
      ...
      if (res.data.success) {
        ...
        toast.error("Error Printing Badge", res.data.message);
      }
    } catch (error) {
      toast.error("Error Printing Badge");
    }
  };
                ...
                <td className="py-2 px-4 border-b flex">
                  {item?.badge_printed ? ( 
                    ...
                    <button
                      ...
                      className="text-white hover:opacity-80 bg-print  rounded-lg px-3 mr-2 flex items-center gap-2"
                    >
